...
router.get("/", asyncHandler(async(req,res)=>{
    ...
    if(latestResumeMeta){
        ...
        const analyses = analysisIds.length ? await Analysis.find({_id: { $in: analysisIds}})
        .select("_id atsScore versionId createdAt")
        .lean(): [];

        const scoredByVersion = new Map(
        ...
        const versionWithScores = versions.map((v)=> ({
            ...
            score: scoredByVersion.get(v._id.toString())??null,
        }));
        ...
        scoreSeries = versionWithScores.filter((v)=> v.score!=null)
        .map((v)=> ({
            label: v.label,
            score: v.score,
            versionId: v.id,
    ...
    //Key Performance Indicator derived from history
    ...
    keywordsMatched: {
        ...
        total:latestAnalysis ? (latestAnalysis.keywordsPresent?.length || 0) + (prevAnalysis?.keywordsPresent?.length || 0) : null,
        delta:latestAnalysis && prevAnalysis ? (latestAnalysis.keywordsPresent?.length || 0) - (prevAnalysis.keywordsPresent?.length || 0) : null,
        spark: keywordsSpark,
    },
    };
