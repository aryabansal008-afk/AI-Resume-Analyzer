...
//For user getting their own data only, keep two checks 
...
router.post("/", uploadPdf("file"), asyncHandler(async(req,res)=>{
    ...
    const title = (req.body.title || "").trim() || req.file.originalname.replace(/\.pdf$/i, "") || "Untitled resume";
...
router.get("/:id/versions/:versionId", validate(
...
router.post("/:id/analyze", analyzeLimiter, validate(idParam, "params"), validate(analyzeBody), asyncHandler(async(req,res)=>{
    const resume = await loadOwnedResume(req);
...
router.get("/:id/analyses", validate(idParam,"params"), asyncHandler(async(req,res)=>{
    const resume = await loadOwnedResume(req);
...
function applyRewritesToText(rawText, rewrites){
    ...
    for(const r of rewrites){
        ...
        if(idx>=0){
            ...
            result+= `\n${r.rewritten}`; //improvement never gets lost
        }
    }
    return result;
}

function patchBulletsInSchema(sections, rewrites){
...
function looksEmpty(sections){
    ...
    const hasIdentify = b.name || b.email || b.title;
    const hasBody = sections.summary ||sections.experience?.length || sections.education?.length || sections.skills?.length;
    return !hasIdentify && !hasBody; 
}

router.post("/:id/rewrite", validate(idParam, "params"), validate(rewriteBody), asyncHandler(async(req,res)=>{
    const resume = await loadOwnedResume(req);
    const analysis = await Analysis.findOne({
    ...
    const selected = req.body.rewriteIds?.length?analysis.bulletRewrites.filter((r)=>
    req.body.rewriteIds.includes(r._id.toString())): analysis.bulletRewrites;
    ...
    const patchedFromBase = patchBulletsInSchema(
    baseVersion.parsedSections, selected);
    ...
    router.get("/:id/diff", validate(idParam, "params"), validate(diffQuery, "query"), asyncHandler(async(req, res) => {
    const resume = await loadOwnedResume(req);
    const [fromV, toV] = await Promise.all([loadVersion( resume._id, req.query.from) , loadVersion(resume._id, req.query.to)]);
